<script lang="ts">
	...
	import { getProductions, deleteProduction, type Production } from '$lib/firebase/productions';
	...
	async function loadData() {
		loading = true;
		try {
			productions = await getProductions();
		} catch {
			error = 'Gagal memuat data production.';
		} finally {
			loading = false;
		}
	}
	...
	async function handleDelete(item: Production) {
		...
		try {
			...
			productions = productions.filter((p) => p.id !== item.id);
		} catch {
			error = 'Gagal menghapus production.';
		} finally {
			deletingId = null;
		}
	}
...
<svelte:head>
	<title>Manajemen Production — Admin Redline</title>
</svelte:head>

<div class="page">
	<div class="page-header">
		<div>
			<h1 class="page-title">Manajemen Production</h1>
			<p class="page-sub">{productions.length} item tersedia</p>
		</div>
		<a href="/admin/productions/create" class="btn-primary">+ Tambah Production</a>
	</div>
	...
	{:else if filtered.length === 0}
		<div class="empty-state">
			{#if searchQuery}
				<p>Tidak ada production yang cocok dengan "<strong>{searchQuery}</strong>"</p>
			{:else}
				<p>Belum ada production. <a href="/admin/productions/create" class="link">Tambah sekarang!</a></p>
			{/if}
		</div>
	{:else}
						...
						<tr>
							...
							<td>
								<div class="action-wrap">
									<a href="/admin/productions/{item.id}/edit" class="action-btn edit">Edit</a>
									<button
