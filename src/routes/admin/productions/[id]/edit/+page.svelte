<script lang="ts">
	...
	import { getProductionById, updateProduction } from '$lib/firebase/productions';
	...
	onMount(async () => {
		try {
			const item = await getProductionById(itemId || '');
			if (!item) {
				goto('/admin/productions');
				return;
			}
			title = item.title;
			...
			errors = { global: 'Gagal memuat data production.' };
		} finally {
			loading = false;
		}
	});
	...
	async function handleSubmit() {
		...
		try {
			...
			successMsg = 'Production berhasil diperbarui! Mengarahkan...';
			setTimeout(() => goto('/admin/productions'), 1500);
		} catch {
...
<svelte:head>
	<title>Edit Production — Admin Redline</title>
</svelte:head>

<div class="page">
	<div class="page-header">
		<div>
			<a href="/admin/productions" class="back-link">← Kembali ke Daftar Production</a>
			<h1 class="page-title">Edit Production</h1>
		</div>
	</div>
	...
	{#if loading}
		<div class="loading-state">Memuat data production...</div>
	{:else}
		<form class="form-grid" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
			...
			<div class="form-col-side">
				<div class="side-card">
					...
					<a href="/admin/productions" class="cancel-link">Batalkan</a>
				</div>

				<div class="side-card">
					<h3 class="side-card-title">Gambar</h3>
					{#if imagePreview}
						...
						{#if imageFile}
							<p class="image-info">
								...
								<span class="compress-note">Akan dikompres otomatis ke ~30-60KB</span>
							</p>
						{/if}
					{:else}
						<label class="image-upload-zone" for="image-input">
							<span class="upload-icon">📷</span>
							<span class="upload-text">Upload gambar baru</span>
