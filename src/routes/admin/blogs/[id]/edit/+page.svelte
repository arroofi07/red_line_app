<script lang="ts">
	...
	import { getBlogById, updateBlog, generateSlug, type Blog } from '$lib/firebase/blogs';
	...
	onMount(async () => {
		try {
			...
			existingImagePath = blog.imagePath;
			imagePreview = blog.imageUrl;
			slugEdited = true; // jangan auto-overwrite slug yang sudah ada
		} catch {
			errors = { global: 'Gagal memuat data blog.' };
		} finally {
			loading = false;
		}
	});

	// Auto-slug hanya jika belum di-edit
	$effect(() => {
		if (!slugEdited && title) {
	...
	function removeImage() {
		imageFile = null;
		imagePreview = existingImageUrl; // kembali ke gambar lama
		originalSize = 0;
	}
		...
		try {
			...
			if (imageFile) {
				...
				if (existingImagePath) {
					await deleteBlogImage(existingImagePath);
				}
				const result = await uploadBlogImage(imageFile, `${blogId}_edit`);
			...
			successMsg = 'Blog berhasil diperbarui! Mengarahkan...';
...
<svelte:head>
	<title>Edit Blog — Admin Redline</title>
</svelte:head>
...
<div class="page">
	...
	{#if loading}
		<div class="loading-state">⏳ Memuat data blog...</div>
	{:else}
		<form class="form-grid" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
			...
			<div class="form-col-main">
				...
				<div class="field">
					<label class="label" for="slug">
						Slug URL
						<span class="label-hint">(hati-hati mengubah slug yang sudah dipublish)</span>
					</label>
					<div class="slug-wrap">
				...
				<div class="field" class:has-error={errors.excerpt}>
					...
					<textarea id="excerpt" bind:value={excerpt} rows="3" class="input textarea"></textarea>
				...
				<div class="field" class:has-error={errors.content}>
					...
					<textarea id="content" bind:value={content} rows="16" class="input textarea content-area"></textarea>
			...
			<div class="form-col-side">
				<div class="side-card">
					<h3 class="side-card-title">Simpan Perubahan</h3>
				...
				<div class="side-card">
					...
					{#if imagePreview}
						<div class="image-preview-wrap">
							...
							{#if imageFile}
								<button type="button" class="remove-image" onclick={removeImage}>↩ Batalkan</button>
							{/if}
						</div>
						{#if imageFile}
							<p class="image-info">
					...
					{:else}
						<label class="image-upload-zone" for="image-input">
							...
							<span class="upload-text">Klik untuk upload gambar baru</span>
							<span class="upload-hint compress-note">🗜️ Dikompres otomatis ke 800px</span>
						</label>
					{/if}
					<label class="replace-btn" for="image-input">
				...
				<div class="side-card">
					...
					<div class="field">
						...
						<input id="author" type="text" bind:value={author} class="input" />
					</div>
					<div class="field">
						<label class="label" for="readtime">Estimasi Baca</label>
						<input id="readtime" type="text" bind:value={readTime} class="input" />
					</div>
				</div>
			</div>
		</form>
...
<style>
	...
	.slug-prefix { padding: 0.7rem 0.625rem; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.1); border-right: none; border-radius: 0.625rem 0 0 0.625rem; font-size: 0.8rem; color: rgba(255,255,255,0.3); white-space: nowrap; flex-shrink: 0; }
	...
	.replace-btn { display: flex; justify-content: center; padding: 0.55rem 0.875rem; background: rgba(255,255,255,0.06); border: 1px solid rgba(255,255,255,0.1); border-radius: 0.5rem; font-size: 0.8rem; color: rgba(255,255,255,0.5); cursor: pointer; transition: all 0.2s; text-align: center; }
	...
	.loading-state { padding: 3rem; text-align: center; color: rgba(255,255,255,0.4); }
